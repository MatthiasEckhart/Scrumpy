...
ownsDocumentOrAdminOrProductOwner = function (userId, doc) {
    return (doc && doc.userId === userId) || Roles.userIsInRole(Meteor.user(), [doc.productId], 'administrator') || Roles.userIsInRole(Meteor.user(), [doc.productId], 'productOwner');
};

ownsDocumentOrProductOwner = function (userId, doc) {
    return (doc && doc.userId === userId) || Roles.userIsInRole(Meteor.user(), [doc.productId], 'productOwner');
};
