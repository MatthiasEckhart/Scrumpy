...
DashboardController = RouteController.extend({
    ...
    findOptions: function () {
        return {sort: {createdAt: -1, updatedAt: -1}, limit: this.limit()};
    },
    waitOn: function () {
        if (this.productIds())
            return [
                ...
                Meteor.subscribe('recentPrivateMessages', Meteor.userId()),
...
Router.map(function () {
        ...
        waitOn: function () {
            if (this.params.startDate && this.params.endDate) {
                return [
                    ...
                    Meteor.subscribe('userProductAuthor', this.params.slug),
                    Meteor.subscribe('sprints', this.params.slug),
                    Meteor.subscribe('userStoriesAdvanced', this.params.slug, moment.utc(this.params.startDate).toDate(), moment.utc(this.params.endDate).toDate())
                ];
            }
            return [
                Meteor.subscribe('singleProduct', this.params.slug),
    ...
    this.route('productInvitations', {
        path: '/product/:slug/invitations',
        onBeforeAction: [productOwnerAdminFilter],
        waitOn: function () {
            return [
                Meteor.subscribe('usernamesRoles'),
